...
@store_product_router.get(BASE + "/{store_id}/{product_id}", response_model=StoreProductSchema)
...
@store_product_router.get(BASE + "/{store_id}", response_model=list[StoreProductSchema])
def get_all_store_products(organization_id: UUID, store_id: UUID) -> list[StoreProductSchema]:
    return store_product_dao.get_all_store_products(store_id)
